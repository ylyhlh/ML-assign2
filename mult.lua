...
function multOneVsAll(mfunc)
   ...
   function mult:g(x)
		 ...
		 local y=torch.Tensor(mult:classes()):fill(0)
...
function multOneVsOne(mfunc)
   ...
   local function procOneVsOne(dataset)
      ...
      for i = 1, dataset:classes() do
				data[i]={}
				for k= i+1, dataset:classes() do
					 -- Create this dataset, with size() method returning the same thing as dataset
					 ------data[i][j] = {size = dataset.size}
					 -- Modify the labels
					 local num=0
					 data[i][k] = { size=function() return num end }
					 for j = 1, dataset:size() do
							-- Create entry
							if dataset[j][2][1] == i then
								num=num+1
								data[i][k][num]={}
								data[i][k][num][1] = dataset[j][1]
								data[i][k][num][2] = torch.ones(1)
							end
							if dataset[j][2][1] == k then
								num=num+1
								data[i][k][num]={}
								data[i][k][num][1] = dataset[j][1]
								data[i][k][num][2] = -torch.ones(1)
  ...
  function mult:g(x)
		 ...
		 local y=torch.Tensor(mult:classes()):fill(0)
		 for i = 1, mult:classes() do
			for k= i+1, mult:classes() do
				if(mult[i][k]:g(x)[1]==1) then
			  		y[i]=y[i]+1
				else
						y[k]=y[k]+1
				end
